...
function buildYearRows(rows: AnalyticsYearRow[]) {
  ...
  for (const row of rows) {
    ...
    if (row.type === "conference_paper")
      current.conference = row.unique_publications;
...
function ModuloAnalitico() {
  ...
  const [topResearchers, setTopResearchers] = useState<
    AnalyticsResearcherRow[]
  >([]);
  const [coauthorNetwork, setCoauthorNetwork] =
    useState<AnalyticsCoauthorNetwork>({
      nodes: [],
      edges: [],
    });
  ...
  useEffect(() => {
    ...
    async function loadAnalyticsData() {
      ...
      try {
        setLoading(true);
        const [summaryData, byYear, byArea, topData, networkData] =
          await Promise.all([
  ...
  const areaData = useMemo<AreaSlice[]>(
    () => buildAreaSlices(areaRows),
    [areaRows],
  );

  const maxBar = Math.max(
    1,
    ...topResearchers.map((researcher) => researcher.authorships),
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      <FiltrosPanel
        ...
        onExportResearchers={() =>
          downloadAnalyticsCSV("researchers", {
            types: selectedTypes.map(typeParam),
            yearFrom: yearRange.from,
            yearTo: yearRange.to,
            area: selectedArea === "Todas as Areas" ? undefined : selectedArea,
      ...
      <main className="ml-[200px] mr-[250px] px-8 py-8">
        ...
        <section className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "COAUTORIAS INTERNAS", value: internalCoauthorships },
            {
              label: "SEM DOI",
              value:
                summary.papers_without_doi +
                summary.conference_papers_without_doi,
            },
          ].map((stat) => (
            <div
              key={stat.label}
              className="rounded-xl border border-border/80 bg-card px-5 py-4 shadow-[var(--shadow-card)]"
            >
              ...
              <p className="mt-1 font-serif text-3xl text-primary">
                {loading
                  ? "..."
                  : Number(stat.value ?? 0).toLocaleString("pt-BR")}
              </p>
            </div>
          ))}
        </section>
          ...
          <div className="grid grid-cols-3 gap-3">
            {Object.entries(summary.productions_by_type).map(
              ([type, values]) => (
                <div
                  key={type}
                  className="rounded-md border border-border/80 bg-secondary p-3"
                >
          ...
          <div className="flex h-64 items-stretch gap-3 border-l border-b border-border pl-2 pb-2 pt-2">
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      height: total > 0 ? `${Math.max(height, 4)}%` : "2px",
                    }}
                  ...
                  >
                    {total > 0 && (
                      <>
                        <div
                          style={{
                            height: `${(row.paper / total) * 100}%`,
                            background: "var(--chart-1)",
                          }}
                        />
                        <div
                          style={{
                            height: `${(row.conference / total) * 100}%`,
                            background: "var(--chart-2)",
                          }}
                        />
                        <div
                          style={{
                            height: `${(row.advising / total) * 100}%`,
            ...
            {!loading && yearData.length === 0 && (
              <p className="self-center text-xs text-muted-foreground">
                Nenhum dado no intervalo selecionado.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="mt-6 rounded-xl border border-border/80 bg-card p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="space-y-2">
            {coauthorNetwork.edges.slice(0, 8).map((edge) => {
              const source = coauthorNetwork.nodes.find(
                (node) => node.id === edge.source,
              );
              const target = coauthorNetwork.nodes.find(
            ...
            {!loading && coauthorNetwork.edges.length === 0 && (
              <p className="text-xs text-muted-foreground">
                Nenhuma coautoria interna encontrada.
              </p>
            )}
          </div>
        </section>
        ...
        <div className="mt-6 flex items-center gap-3">
          ...
          <button
            onClick={() =>
              downloadAnalyticsCSV("report", {
                types: selectedTypes.map(typeParam),
                yearFrom: yearRange.from,
                yearTo: yearRange.to,
                area:
                  selectedArea === "Todas as Areas" ? undefined : selectedArea,
              })
            }
            className="flex items-center gap-2 rounded-md border border-primary/40 bg-primary/10 px-4 py-2 text-sm text-primary transition-colors hover:bg-primary/20"
          >
